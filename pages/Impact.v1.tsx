...
export const ImpactV1: React.FC = () => {
  ...
  const activeArea = selectedArea 
    ? MOCK_AREAS.find(a => a.id === selectedArea) 
    : null;
  ...
  const GaugeChart = ({ data, color }: { data: any[], color: string }) => (
    <div className="relative h-[160px] w-full flex justify-center items-end">
        ...
        <div className="relative z-10 text-center mb-0 pb-2">
            ...
            <span className="block text-xs font-bold text-slate-400 uppercase tracking-wider mt-1">Contribution</span>
        </div>
    </div>
  );
    ...
    <div className="animate-fade-in bg-slate-50 min-h-screen">
      <section className="bg-brand-blue pt-32 pb-32 text-white">
        <div className="max-w-7xl mx-auto px-4 text-center">
            ...
            <p className="text-blue-200">Tracking our progress in real-time across the Braamfontein Spruit and Kliprivier.</p>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-20">
        <div className="bg-white rounded-2xl shadow-xl p-6 mb-12 border border-slate-100">
            <div className="flex flex-col md:flex-row justify-between items-center mb-8">
                <div>
                    ...
                    <p className="text-sm text-slate-500 mt-1">Monthly breakdown of waste removal efforts (kg)</p>
                </div>
            ...
            <div className="h-[400px] w-full">
                <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={CHART_DATA} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                        ...
                        <Bar dataKey="kgs" name="Waste Collected (Kgs)" fill={selectedArea ? '#009B4D' : '#003366'} radius={[20, 20, 20, 20]} barSize={40} />
                    </BarChart>
                </ResponsiveContainer>
            </div>
        </div>
        ...
        <section className="bg-white py-24 px-4 sm:px-6 lg:px-8">
            <div className="text-center max-w-3xl mx-auto mb-16">
                ...
                <p className="text-lg text-slate-600">Our commitment goes deeper. We actively promote sustainable practices and eco-friendly solutions.</p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12">
                <div className="bg-white rounded-3xl p-8 shadow-xl border border-slate-100 flex flex-col items-center text-center">
                    ...
                    <p className="text-slate-600 mt-4 mb-8">Revitalizing the river banks and reducing plastic waste.</p>
                    <Link to="/get-involved" className="px-8 py-3 bg-slate-100 text-brand-blue font-bold rounded-full">Learn more</Link>
                </div>
                <div className="bg-white rounded-3xl p-8 shadow-xl border border-slate-100 flex flex-col items-center text-center">
                    <GaugeChart data={getGaugeData('soweto')} color="#009B4D" />
                    <h3 className="text-2xl font-bold text-brand-blue mt-4">Kliprivier</h3>
                    <p className="text-slate-600 mt-4 mb-8">Creating sustainable employment and educational opportunities.</p>
                    <Link to="/get-involved" className="px-8 py-3 bg-slate-100 text-brand-blue font-bold rounded-full">Learn more</Link>
                </div>
            </div>
        </section>
