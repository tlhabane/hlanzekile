        ...
        <div className="h-screen w-full bg-slate-100 flex overflow-hidden pt-20 animate-fade-in">
            ...
            <div className="w-[450px] bg-white h-full border-r border-slate-100 shadow-2xl z-20 overflow-y-auto flex flex-col">
                <div className="p-8 border-b border-slate-50">
                    <Link
                        to="/sponsor-a-grid"
                        className="inline-flex items-center text-slate-400 hover:text-brand-blue mb-8 text-xs font-black uppercase tracking-widest gap-2"
                    >
                        <ChevronLeft size={16} /> Back to Selection
                    </Link>
                    <h1 className="text-5xl md:text-7xl font-bold text-white mb-8 tracking-tight">{area.fullName}</h1>
                    <p className="text-brand-green font-bold text-xs uppercase tracking-widest">
                        ...
                        <div className="space-y-8">
                            <div className="p-6 bg-blue-50 rounded-3xl border border-blue-100">
                                ...
                                <p className="text-sm text-slate-600 leading-relaxed">
                                    Select a numbered grid along the river on the map to view sponsorship options.
                                    Available grids are outlined in green.
                                </p>
                            </div>
                        ...
                        <div className="space-y-8 animate-slide-up">
                            <div className="flex justify-between items-center">
                                <h3 className="text-2xl font-black text-brand-blue uppercase tracking-tight">
                                    Grid #{selectedGrid.number}
                                </h3>
                                <button
                                    onClick={() => setSelectedGrid(null)}
                                    className="p-2 bg-slate-100 rounded-full text-slate-400 hover:text-red-500 transition"
                                >
                                    <X size={20} />
                                </button>
                            </div>
                            ...
                            <div className="aspect-square bg-slate-50 rounded-3xl overflow-hidden relative">
                                <img
                                    src={`https://placehold.co/800x800/webp?text=Grid+Surface+#${selectedGrid.number}`}
                                    alt="Grid View"
                                    className="w-full h-full object-cover"
                                />
                                <div className="absolute top-4 right-4 px-3 py-1 bg-brand-green text-white text-[10px] font-black rounded-full uppercase tracking-widest">
                                    Pristine Potential
                                </div>
                            </div>
                            ...
                            <div className="space-y-4">
                                ...
                                >
                                    <div className="flex justify-between items-center mb-1">
                                        <span className="font-black text-brand-blue uppercase text-sm">
                                            Monthly Stewardship
                                        </span>
                                        <span className="font-black text-brand-green text-lg">R500 / mo</span>
                                    </div>
                                    <p className="text-xs text-slate-500">
                                        Continuous field crew support. Invoiced monthly.
                                    </p>
                                </div>
                                ...
                                >
                                    <div className="flex justify-between items-center mb-1">
                                        ...
                                        <div className="text-right">
                                            <span className="font-black text-brand-yellow text-lg block">
                                                R5000 / yr
                                            </span>
                    ...
                    <div className="relative ml-auto w-full max-w-md bg-white h-full shadow-2xl flex flex-col animate-slide-left">
                        ...
                        <div className="p-8 flex-grow overflow-y-auto space-y-6">
                            {cart.map((item) => (
                                ...
                                <div
                                    key={item.id}
                                    className="p-6 bg-slate-50 rounded-3xl border border-slate-100 flex justify-between items-center"
                                >
                                    <div>
                                        {/* Fixed: Conditional check for gridNumber or display areaName if bulk */}
                                        <h4 className="font-black text-brand-blue text-sm uppercase">
                                            {item.gridNumber ? `Grid #${item.gridNumber}` : item.areaName}
                                        </h4>
                                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                                            {item.areaName}
                                        </p>
                                        <p className="text-xs font-bold text-brand-green mt-2 capitalize">
                                            {item.plan} Plan
                                        </p>
                                    </div>
                                    <div className="text-right">
                        ...
                        <div className="p-8 bg-slate-50 border-t border-slate-100">
                            <div className="flex justify-between items-center mb-8">
                                ...
                                <span className="text-3xl font-black text-brand-blue tracking-tighter">
                                    R {cart.reduce((sum, item) => sum + item.price, 0)}
                                </span>
                            </div>
                            <Link
                                to="/checkout"
